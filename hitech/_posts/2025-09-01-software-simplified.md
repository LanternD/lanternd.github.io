...
　所以目前，我在新买的 Mac Studio 上没有再安装 Spacemacs 了。
...
　我从 Chrome 刚发布不久开始就是它的忠实用户，甚至有段时间（2010 年前后）觉得其他浏览器都是垃圾（当时确实是，现在此观点不可取）。它很强大，生态丰富，用着也顺手。听起来好像确实也是可以用下去的。

　之所以离开 Chrome，原因是我工作中用 Safari 越来越多了。Safari 用的内存少很多，加载速度也快，界面也足够简洁。而且有一段时间我在公司电脑用 Safari，回家用 Chrome，也就有了更多对比的机会。有一段时间 Chrome 经常让我更新，我也不知道它更新了什么，就是觉得很多 Tab 打开了又关掉又打开特别烦。另一方面，Safari 用起来也还挺顺手，我常用的几个 Chrome 扩展也都能在 Safari 找到平替。既然 Safari 有过之而无不及，那我心中的天平就开始倾斜了。
...
　2025.11.30 更新：如果在 macOS 的生态里面，Safari 的密码、个人信息等补全丝滑得多。还有比如验证码，直接从短信或邮件那里读取，然后推送到 Safari 页面里的输入框，从头到尾不用切换任何窗口，丝滑到没朋友。Chrome 在这一块还是吃了点亏。所以我更离不开 Safari 了。
...
　告别的原因只有一个，**没需求了**。反正工作以后不写论文了，没什么需要排版的东西。公司的文档都是云端笔记系统。个人需要记录的东西简单的 Markdown 语法（比如 `-` 来开始列表，`#` 来区分标题）就搞定了。在 Note 应用里开个文档记下来就行，反正也是云端同步到各个设备上。
...
　上面列举的东西是我彻底不想再安装的，但是 Sublime Text 是我唯一 **想告别却没成功的**。

　Sublime Text 在我每台桌面设备都留下过痕迹。也是我曾经的装机必装软件。它编辑功能极其强大，还有丰富的包管理系统和生态。第一次用它多点编辑的功能真是眼前一亮。它在格式化文本方面真的太香了。
...
　于是我现在尴尬地两个软件都装上了。期待有朝一日 Zed 把这个功能加上，我大概就能和 Sublime Text 真正告别了。现在 Sublime Text 唯一使用场景就是把博客文章粘贴进去，跑一个 Pangu 脚本自动化加上空格，然后再复制出来贴回原来的编辑器。
