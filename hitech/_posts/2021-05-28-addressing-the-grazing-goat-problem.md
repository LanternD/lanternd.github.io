...
　草场的圆心是 $O_1$ ，羊活动的圈子的半径是 $O_2$。两圆的交点为 $A$ 和 $B$ 。$C$ 是两圆心连线和 $\odot O_2$ 的交点。
...
　到这里就全部解决了。这个问题不算难，不过对我来说意义更大的是把「**问题抽象 - 公式化 - 数值计算**」这个流程走了一遍，把很多很久不用的知识串联了一下。
...
　这个问题就像把上面 $O_1$ 和 $O_2$ 按对称轴旋转一圈形成的球体的。
...
　但是从二维到三维不具有相似性。这个绳长需要更长一些才能满足要求。WikiPedia上用棱镜体积公式得到如下方程：
...
　数学家们最擅长干这个：把二维、三维的问题继续推广到高维，求出半径 $r$ 与维度 $n$ 的关系。

　**假设有个高维生物拴在一个高维球体的内部，请问绳长是多少的时候，高维生物的活动范围是高维球体体积的一半？**
